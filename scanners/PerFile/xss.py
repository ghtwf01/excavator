...
class MyHTMLParser(HTMLParser):
    ...
    def check_body_xss(self, url, value, headers):
        if self.body_tag == "script":
            ...
            url_body2 = get_replaced_url(
                url, value, '</' + self.body_tag + '>' + '<arcbvyf>')
            res2 = ""
            try:
                    ...
                    if len(self.position) != 0:
                        if len(self.position) == 1:
                            if self.position[0] == "value" or self.position[0] == "href":
                                self.check_value_xss(
                                    url, dict[key][0], self.position[0], headers=request.headers)
                            if self.position[0] == "body":
                                self.check_body_xss(
                                    url, dict[key][0], headers=request.headers)
                            if self.position[0] == "comment":
                                self.check_comment_xss(
                                    url, dict[key][0], headers=request.headers)
