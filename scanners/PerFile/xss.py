...
class MyHTMLParser(HTMLParser):
    ...
    def check_value_xss(self, url, value, type):
        url_value1_single = get_replaced_url(url, value, '"psafq="g')
        res1_single = requests.get(url_value1_single).text
        ...
        res2_single = requests.get(url_value2_single).text
        ...
        res3_single = requests.get(url_value3_single).text
        ...
        res4_single = requests.get(url_value4_single).text
    ...
    def check_body_xss(self, url, value):
        if self.body_tag == "script":
            ...
            res2 = requests.get(url_body2).text
            ...
            if soup.find("arcbvyf"):
                print("payload："+"</"+self.body_tag+">"+"<img src=1>")
                vuln_print(url_body2, "xss", vuln_level["xss"], self.method)
    def check_comment_xss(self, url, value):
                    ...
                    if len(self.position) != 0:
                        if len(self.position) == 1:
                            if self.position[0] == "value" or self.position[0] == "href":
                                self.check_value_xss(url, dict[key][0], self.position[0])
                            if self.position[0] == "body":
                                self.check_body_xss(url, dict[key][0])
                            if self.position[0] == "comment":
                                self.check_comment_xss(url, dict[key][0])