...
class Unauth_Check:
    def check_get_unauth(self, url, req_headers, method, response_text):
    ...
    def check_post_urlencode_unauth(self, url, req_headers, method, body, response_text):
        ...
        for key, value in req_headers.items():
            if key.lower() in ["cookie", "token", "auth"]:
                ...
                if similar(html, response_text) >= 0.95:
                    print(url+" 存在未授权访问")
                    vuln_print(url, "unauth", vuln_level["unauth"], method, body)

    def check_post_json_unauth(self, url, req_headers, method, body, response_text):
